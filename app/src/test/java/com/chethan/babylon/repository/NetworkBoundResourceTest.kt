...
import com.chethan.babylon.util.CountingAppExecutors
...
class NetworkBoundResourceTest(private val useRealExecutors: Boolean) {
    ...
    private lateinit var handleSaveCallResult: (userPost) -> Unit

    private lateinit var handleShouldMatch: (userPost?) -> Boolean

    private lateinit var handleCreateCall: () -> LiveData<ApiResponse<userPost>>

    private val dbData = MutableLiveData<userPost>()

    private lateinit var networkBoundResource: NetworkBoundResource<userPost, userPost>
    ...
    fun init() {
        val appExecutors = if (useRealExecutors)
            ...
            InstantAppExecutors()
        networkBoundResource = object : NetworkBoundResource<userPost, userPost>(appExecutors) {
            override fun saveCallResult(item: userPost) {
                handleSaveCallResult(item)
            }

            override fun shouldFetch(data: userPost?): Boolean {
            ...
            override fun loadFromDb(): LiveData<userPost> {
                return dbData
            }

            override fun createCall(): LiveData<ApiResponse<userPost>> {
    ...
    fun failureFromNetwork() {
        ...
        handleCreateCall = { ApiUtil.createCall(Response.error<userPost>(500, body)) }
    ...
    fun dbSuccessWithFetchFailure() {
        ...
        val body = ResponseBody.create(MediaType.parse("text/html"), "error")
        val apiResponseLiveData = MutableLiveData<ApiResponse<userPost>>()
        ...
        apiResponseLiveData.value = ApiResponse.create(Response.error<userPost>(400, body))
        drain()
        assertThat(saved.get(), `is`(false))
        verify(observer).onChanged(Resource.error("error", dbValue))

        val dbValue2 = userPost(2)
    ...
    private data class userPost(var value: Int)
